"""
Instrumented GRPO training script for credit assignment analysis.

This script extends the base training to:
1. Use grpo_instrumented.py with per-token logging
2. Save credit assignment logs periodically
3. Track additional metrics for analysis
"""
...
from grpo_instrumented import rollout, update_policy, get_credit_logger, reset_credit_logger
...
def main(config_path: str):
    ...
    credit_log_dir = Path(config["training"].get("credit_log_dir", "credit_logs"))
    ...
    print(f"Starting instrumented training run: {current_time}")
    print(f"Credit logs will be saved to: {credit_log_dir}")
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        # INSTRUMENTED: Pass step number and enable logging
        results = update_policy(
            ...
            device=device,
            dtype=dtype,
            step=step,  # Pass step for logging
            log_tokens=True,  # Enable per-token logging
        )
        ...
        # Additional credit assignment stats
        ...
        print(
            ...
            f"num_finished: {num_finished_episodes}, "
            f"mean_len: {mean_response_len:.2f}, "
            f"tokens_logged: {num_tokens_logged}"
        )
        ...
        for i, episode in enumerate(episodes[:4]):  # Log first 4 episodes
    ...
    print(f"\nTraining complete! Final credit logs saved to {final_log_path}")
